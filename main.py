...
def paper_trade(
    symbols: str = typer.Option("AAPL,MSFT,GOOGL", help="逗号分隔的股票代码"),
    strategy: str = typer.Option("composite", help="策略: ma/rsi/bb/composite"),
    initial_cash: float = typer.Option(100_000.0, help="初始资金（首次运行有效）"),
):
    """每日模拟交易：自动扫描信号并记录假设买卖，状态持久保存。"""
    console.print(
        "\n[bold yellow]⚠ 警告：paper-trade 使用单股择时策略，未经验证有效。[/bold yellow]\n"
        "  建议改用 [bold cyan]factor-paper[/bold cyan]（截面选股，已回测验证 +110% vs SPY +54%）\n"
        "  继续运行中...\n"
    )
    from paper_trading import PaperRunner
    symbol_list = [s.strip() for s in symbols.split(",")]
    strat = _get_strategy(strategy)  # paper-trade实时模式，不注入固定日期
    runner = PaperRunner(symbol_list, strat, initial_cash=initial_cash)
    report = runner.run()

    console.print(f"\n[bold green]── 今日模拟交易日报 {report['date']} ──[/bold green]")
    console.print(f"  总资产: {report['total_equity']:,.2f}  现金: {report['cash']:,.2f}")
    console.print(f"  累计收益: [{'green' if report['total_return'] >= 0 else 'red'}]{report['total_return']:.2%}[/]")

    if report["executed_trades"]:
        console.print("\n  [bold]今日成交:[/bold]")
        for t in report["executed_trades"]:
            color = "red" if t["side"] == "BUY" else "green"
            console.print(f"    [{color}]{t['side']}[/] {t['symbol']} x{t['qty']} @ {t['price']:.2f}  {t['reason']}")
    else:
        console.print("  今日无成交")
    ...
    if report["positions"]:
        ...
        for sym, p in report["positions"].items():
            console.print(f"    {sym}  {p['qty']}股  成本:{p['avg_cost']:.2f}")
...
def alpaca_paper(
    ...
    rebalance_days: int   = typer.Option(30,       help="调仓周期（日历日）"),
    spy_filter: bool      = typer.Option(True, "--spy-filter/--no-spy-filter", help="SPY跌破MA200时清仓"),
...
@app.command(name="paper-backfill")
def paper_backfill(
    symbols: str = typer.Option("AAPL,MSFT,GOOGL", help="逗号分隔的股票代码"),
    strategy: str = typer.Option("composite"),
    start: str = typer.Option("2024-01-01", help="回填开始日期"),
    end: str = typer.Option(str(date.today()), help="回填结束日期"),
    initial_cash: float = typer.Option(100_000.0, help="初始资金"),
):
    """用历史数据快速模拟多天交易，几秒内看完数月结果。"""
    from paper_trading import PaperRunner, PaperState
    import json
    from pathlib import Path

    # 重置状态文件，重新开始
    state_file = Path("data/paper_trading.json")
    ...
    console.print(f"[green]开始回填: {start} ~ {end}  股票: {symbol_list}[/green]")
    reports = runner.backfill(start, end)

    if not reports:
        console.print("[red]回填失败，无数据[/red]")
        return

    console.print(f"\n[bold green]── 回填完成，共 {len(reports)} 个交易日 ──[/bold green]")

    # 统计
    all_trades = runner.state.all_trades()
    buys  = [t for t in all_trades if t["side"] == "BUY"]
    sells = [t for t in all_trades if t["side"] == "SELL"]
    final = reports[-1]
    total_ret = final["return"]

    console.print(f"  初始资金:  {initial_cash:>12,.2f}")
    console.print(f"  最终资产:  {final['equity']:>12,.2f}")
    console.print(f"  累计收益: [{'green' if total_ret >= 0 else 'red'}]{total_ret:+.2%}[/]")
    console.print(f"  买入次数:  {len(buys)}  卖出次数: {len(sells)}")
    ...
    console.print(f"  最大回撤: [red]{max_dd:.2%}[/]")
    ...
    prev = initial_cash
    for m, eqs in monthly.items():
        ret = (eqs[-1] - prev) / prev
        color = "green" if ret >= 0 else "red"
        console.print(f"    {m}  [{color}]{ret:+.2%}[/]")
        prev = eqs[-1]

    # 最近10笔交易
    if all_trades:
        console.print(f"\n  [bold]最近10笔交易:[/bold]")
        table = Table("日期", "代码", "方向", "价格", "数量", "原因")
        for t in all_trades[-10:]:
            color = "red" if t["side"] == "BUY" else "green"
            table.add_row(t["date"], t["symbol"],
                          f"[{color}]{t['side']}[/]",
                          f"{t['price']:.2f}", str(t["qty"]), t["reason"][:35])
        console.print(table)


@app.command(name="paper-status")
def paper_status(
    html: bool = typer.Option(True, help="生成HTML图表并打开浏览器"),
):
    """查看模拟盘当前状态：持仓、历史交易、每日盈亏曲线。"""
    from paper_trading import PaperState
    state = PaperState()

    snaps = state.daily_equity()
    if not snaps:
        console.print("[yellow]还没有模拟交易记录，先运行 paper-trade 或 paper-backfill[/yellow]")
        return
    ...
    console.print(f"\n[bold green]── 模拟盘总览 ──[/bold green]")
    console.print(f"  初始资金:  {initial:>12,.2f}")
    console.print(f"  当前资产:  {snaps[-1]['equity']:>12,.2f}")
    console.print(f"  累计收益: [{'green' if total_ret >= 0 else 'red'}]{total_ret:+.2%}[/]")
    console.print(f"  最大回撤: [red]{max_dd:.2%}[/]")
    console.print(f"  运行天数:  {len(snaps)} 天  买入:{len(buys)}次  卖出:{len(sells)}次")

    pos = state.all_positions()
    if pos:
        console.print("\n  [bold]当前持仓:[/bold]")
        for sym, p in pos.items():
            console.print(f"    {sym}  {p['qty']}股  均价:{p['avg_cost']:.2f}")
    ...
    if html and snaps:
        ...
        returns = [(e - initial) / initial * 100 for e in equity]

        # 回撤序列
        pk, drawdowns = equity[0], []
        for e in equity:
            pk = max(pk, e)
            drawdowns.append((e - pk) / pk * 100)

        fig = make_subplots(rows=2, cols=1, shared_xaxes=True,
                            row_heights=[0.65, 0.35],
                            subplot_titles=("模拟盘资产曲线", "回撤"))

        fig.add_trace(go.Scatter(x=dates, y=equity, name="总资产",
        ...
        fig.add_trace(go.Scatter(x=dates, y=drawdowns, name="回撤%",
                                 line=dict(color="#ff4d4d", width=1.5),
                                 fill="tozeroy", fillcolor="rgba(255,77,77,0.15)"), row=2, col=1)

        # 买卖标记
        buy_dates  = [t["date"] for t in trades if t["side"] == "BUY"]
        sell_dates = [t["date"] for t in trades if t["side"] == "SELL"]
        eq_map = {s["date"]: s["equity"] for s in snaps}
        buy_eq  = [eq_map.get(d, initial) for d in buy_dates]
        sell_eq = [eq_map.get(d, initial) for d in sell_dates]
        ...
        fig.update_layout(
            title=f"模拟盘报告  累计收益:{total_ret:+.2%}  最大回撤:{max_dd:.2%}",
            template="plotly_dark", height=650,
            legend=dict(orientation="h", y=1.05),
        )

        out = "logs/paper_status.html"
        Path(out).write_text(fig.to_html(full_html=True), encoding="utf-8")
        console.print(f"\n[green]HTML报告已保存: {out}[/green]")
        subprocess.run(["open", out])
...
def scan(
    ...
    use_ic: bool = typer.Option(False, "--use-ic", help="使用ic-tune保存的权重（data/factor_weights.json）"),
):
...
def select_stocks_cmd(
    ...
    console.print(table)
    syms = ",".join(r["symbol"] for r in results)
    cmd = f"python3 main.py paper-backfill --symbols {syms} --start 2025-01-01 --end {date.today()} --strategy factor"
    console.print(f"\n[bold]用这些股票跑回填:[/bold]")
    print(cmd)
...
def paper_report(
    output: str = typer.Option("", help="HTML输出路径（空=自动命名）"),
    benchmark: str = typer.Option("^GSPC", help="Benchmark index (^GSPC=S&P500, ^GSPTSE=TSX)"),
):
    """模拟盘绩效报告：资金曲线 vs 基准 + 胜率 + 夏普比率 + 当前持仓。"""
    import json
    import numpy as np
    import pandas as pd
    import plotly.graph_objects as go
    ...
    if not state_file.exists():
        console.print("[red]未找到模拟盘数据，请先运行 paper-backfill[/red]")
        raise typer.Exit(1)

    with open(state_file) as f:
        state = json.load(f)

    daily_equity = state.get("daily_equity", [])
    if len(daily_equity) < 2:
        console.print("[red]数据不足（至少需要2天记录）[/red]")
        raise typer.Exit(1)
    ...
    total_days = len(equity_s)
    years = max(total_days / 252, 0.01)
    total_ret = equity_s.iloc[-1] / initial_cash - 1
    annual_ret = (1 + total_ret) ** (1 / years) - 1
    roll_max = equity_s.cummax()
    drawdown_s = (equity_s - roll_max) / roll_max
    ...
    sortino = float(excess.mean() / downside.std() * np.sqrt(252)) if len(downside) > 1 and downside.std() > 1e-8 else 0.0
    calmar = annual_ret / abs(max_dd) if abs(max_dd) > 1e-8 else 0.0

    # ── 交易统计 ──
    trades = state.get("trades", [])
    sells = [t for t in trades if t.get("side") == "SELL" and t.get("avg_cost")]
    pnls_pct = [(t["price"] - t["avg_cost"]) / t["avg_cost"] for t in sells]
    wins = [p for p in pnls_pct if p > 0]
    losses_abs = [abs(p) for p in pnls_pct if p <= 0]
    win_rate = len(wins) / len(pnls_pct) if pnls_pct else 0.0
    avg_win = sum(wins) / len(wins) if wins else 0.0
    avg_loss = sum(losses_abs) / len(losses_abs) if losses_abs else 0.0
    profit_factor = (sum(wins) / sum(losses_abs)) if losses_abs and sum(losses_abs) > 1e-8 else float("inf")

    # ── 控制台输出 ──
    m_table = Table(title="模拟盘绩效报告", show_lines=True, min_width=40)
    m_table.add_column("指标", style="bold", width=14)
    m_table.add_column("数值", width=14)

    def _color(val: float, good_above: float, bad_below: float) -> str:
        if val >= good_above:
            return "green"
        if val <= bad_below:
            return "red"
    ...
    rows = [
        ("总收益率",   f"{total_ret:+.2%}",   _color(total_ret,  0.1,  0.0)),
        ("年化收益率", f"{annual_ret:+.2%}",   _color(annual_ret, 0.1,  0.0)),
        ("最大回撤",   f"{max_dd:.2%}",        _color(-max_dd,    0.0, -0.15)),
        ("夏普比率",   f"{sharpe:.3f}",        _color(sharpe,     1.0,  0.5)),
        ("索提诺比率", f"{sortino:.3f}",       _color(sortino,    1.5,  0.5)),
        ("卡玛比率",   f"{calmar:.3f}",        _color(calmar,     1.0,  0.3)),
        ("胜率",       f"{win_rate:.1%}",      _color(win_rate,   0.55, 0.4)),
        ("平均盈利",   f"{avg_win:.2%}",       _color(avg_win,    0.08, 0.0)),
        ("平均亏损",   f"-{avg_loss:.2%}",     "red" if avg_loss > 0.05 else "green"),
        ("盈亏比",     f"{profit_factor:.2f}" if profit_factor != float("inf") else "∞",
                                               _color(profit_factor, 2.0, 1.0)),
        ("总交易次数", str(len(pnls_pct)),     "white"),
        ("当前净值",   f"¥{equity_s.iloc[-1]:,.0f}", "white"),
        ("持仓天数",   str(total_days),        "white"),
    ]
    for name, val, color in rows:
        m_table.add_row(name, f"[{color}]{val}[/]")
    console.print(m_table)

    # 当前持仓
    positions = state.get("positions", {})
    if positions:
        p_table = Table(title="当前持仓", show_lines=True)
        p_table.add_column("代码", style="cyan")
        p_table.add_column("数量", justify="right")
        p_table.add_column("成本价", justify="right")
        p_table.add_column("市值估算", justify="right")
        for sym, pos in positions.items():
            mv = pos["qty"] * pos["avg_cost"]
            p_table.add_row(sym, str(pos["qty"]), f"¥{pos['avg_cost']:.2f}", f"¥{mv:,.0f}")
    ...
    benchmark_s = None
    try:
        idx_df = fetcher.get_index_kline(benchmark, start=dates[0], end=dates[-1])
        if not idx_df.empty:
            idx_close = idx_df["close"].reindex(pd.to_datetime(dates)).ffill().dropna()
            benchmark_s = idx_close / idx_close.iloc[0] * initial_cash
    except Exception as e:
    ...
    fig = make_subplots(
        ...
        subplot_titles=("资金曲线 vs 基准", "回撤"),
    )

    fig.add_trace(go.Scatter(
        x=equity_s.index, y=equity_s.values,
        name="模拟盘", line=dict(color="#ef232a", width=2),
    ), row=1, col=1)

    if benchmark_s is not None:
        fig.add_trace(go.Scatter(
            x=benchmark_s.index, y=benchmark_s.values,
            name="S&P 500（归一化）", line=dict(color="#aaa", width=1.5, dash="dot"),
        ), row=1, col=1)

    fig.add_trace(go.Scatter(
        x=drawdown_s.index, y=drawdown_s.values * 100,
        name="回撤%", fill="tozeroy", fillcolor="rgba(239,35,42,0.2)",
        line=dict(color="#ef232a", width=1),
    ), row=2, col=1)

    fig.update_layout(
        title=f"模拟盘绩效报告  总收益 {total_ret:+.2%}  夏普 {sharpe:.2f}  最大回撤 {max_dd:.2%}",
    ...
    out = output or f"logs/paper_report_{dates[-1]}.html"
...
@app.command(name="ic-tune")
def ic_tune(
    universe: str = typer.Option("sp500", help="股票池: sp500 | tsx60 | 逗号分隔的代码"),
    start: str = typer.Option("2022-01-01", help="IC分析起始日期"),
    forward_days: int = typer.Option(5, help="预测未来N日收益"),
    max_stocks: int = typer.Option(80, help="最多使用多少只股票（越多越准但越慢）"),
    n_splits: int = typer.Option(3, help="稳定性检验：将时间轴分N段分别算IC"),
):
    """
    截面IC调权：用股票池里所有股票同时计算因子截面相关，
    比单股IC统计可靠得多。自动保存权重到 data/factor_weights.json。
    首次运行需拉取历史数据，约5-15分钟；之后有缓存会很快。
    """
    import json
    from factor import FactorEngine
    from factor.ic import CrossSectionalICAnalyzer
    from data.stock_selector import get_sp500_symbols

    fetcher = DataFetcher(use_cache=True)
    engine = FactorEngine()
    analyzer = CrossSectionalICAnalyzer(engine)

    # 确定股票池
    if universe.lower() in ("sp500", "tsx60", "nasdaq100"):
        if universe.lower() == "tsx60":
            from data.stock_selector import get_tsx60_symbols
            all_syms = get_tsx60_symbols()
        else:
            all_syms = get_sp500_symbols()
        if not all_syms:
            console.print(f"[red]获取 {universe} 列表失败[/red]")
            raise typer.Exit(1)
        import random
        random.seed(42)
        symbols_to_use = random.sample(all_syms, min(max_stocks, len(all_syms)))
        console.print(f"[cyan]从 {len(all_syms)} 只成分股中随机抽取 {len(symbols_to_use)} 只[/cyan]")
    else:
        symbols_to_use = [s.strip() for s in universe.split(",")]
        console.print(f"[cyan]使用自定义股票池: {len(symbols_to_use)} 只[/cyan]")
    ...
    stock_data: dict = {}
    failed = 0
    console.print(f"[bold]拉取历史数据（{start} ~ {end}）...[/bold]")
    for i, sym in enumerate(symbols_to_use):
        try:
            ...
            failed += 1
        if (i + 1) % 20 == 0:
            console.print(f"  已拉取 {i+1}/{len(symbols_to_use)}，有效 {len(stock_data)} 只")

    if len(stock_data) < 10:
        console.print(f"[red]有效数据不足（{len(stock_data)}只），无法做截面IC[/red]")
        raise typer.Exit(1)

    console.print(f"\n[green]数据就绪：{len(stock_data)} 只股票[/green]（失败 {failed} 只）")

    # 拉取基本面数据（yfinance 当前快照）
    fundamentals: dict = {}

    console.print("[bold]拉取基本面数据（yfinance 当前快照）...[/bold]")
    fund_ok = 0
    for sym in list(stock_data.keys()):
        try:
            fd = fetcher.get_fundamentals(sym)
            if fd and any(fd.get(k) for k in ("roe", "net_profit_growth", "debt_ratio")):
                fundamentals[sym] = fd
                fund_ok += 1
        except Exception:
            pass
    console.print(f"  基本面数据：{fund_ok}/{len(stock_data)} 只有效")

    # 截面IC分析
    with console.status(f"[bold]计算截面IC（{len(stock_data)}只 × 时间轴）...[/bold]"):
        result = analyzer.run(
            stock_data, forward_days=forward_days,
            fundamentals=fundamentals,
        )

    if result.n_periods < 5:
        console.print("[red]有效时间点不足，请延长 start 日期或增加股票数[/red]")
        raise typer.Exit(1)

    # 显示主IC结果
    df_ic = result.summary_df()
    table = Table(
        title=f"截面IC分析（{len(stock_data)}只股票  {result.n_periods}个时间点  预测{forward_days}日）",
        show_lines=True,
    )
    ...
    for _, row in df_ic.iterrows():
        verdict = str(row["评价"])
        color = "green" if "★" in verdict else ("yellow" if "△" in verdict else "dim")
        table.add_row(*[f"[{color}]{v}[/]" if col == "评价" else str(v)
                        for col, v in row.items()])
    console.print(table)

    # 稳定性检验（分段IC方向是否一致）
    console.print(f"\n[bold]稳定性检验（时间轴分{n_splits}段）...[/bold]")
    with console.status("计算各时间段IC..."):
        split_results = analyzer.rolling_stability(
            stock_data, forward_days=forward_days, n_splits=n_splits,
            fundamentals=fundamentals,
        )

    if split_results:
        factor_names = list(engine.weights.keys())
        stab_table = Table(title="分段IC均值（方向一致=因子稳定）", show_lines=True)
        stab_table.add_column("因子")
        for r in split_results:
            stab_table.add_column(r.symbol)
        stab_table.add_column("方向一致?")

        for f in factor_names:
            vals = [r.ic_mean.get(f, 0.0) for r in split_results]
            signs = [1 if v > 0.005 else (-1 if v < -0.005 else 0) for v in vals]
            non_zero = [s for s in signs if s != 0]
            consistent = len(set(non_zero)) <= 1 if non_zero else False
            flag = "[green]✓[/green]" if consistent else "[red]✗[/red]"
            stab_table.add_row(
                f, *[f"{v:+.4f}" for v in vals], flag
            )
        console.print(stab_table)
        console.print("[dim]方向一致 = 因子在不同时期IC符号相同，更可信[/dim]")

    # 生成权重
    weights = result.suggest_weights()
    if not weights:
        console.print("[yellow]IC太弱，无法生成可用权重[/yellow]")
        raise typer.Exit(0)
    ...
    out_path = Path("data/factor_weights.json")
    ...
    w_table = Table(title="新因子权重（已保存）", show_lines=True)
    w_table.add_column("因子")
    w_table.add_column("权重", justify="right")
    w_table.add_column("方向")
    for k, v in sorted(weights.items(), key=lambda x: abs(x[1]), reverse=True):
        if abs(v) < 0.001:
            direction = "[dim]无效[/dim]"
        elif v > 0:
            direction = "[green]做多[/green]"
        else:
            direction = "[red]反向[/red]"
        w_table.add_row(k, f"{v:+.4f}", direction)
    console.print(w_table)
    console.print(f"\n[green]权重已保存到 {out_path}[/green]")
    console.print("用 [bold]scan --use-ic[/bold] 或 [bold]select-stocks[/bold] 自动加载这些权重")


# ── 因子IC有效性分析 ──────────────────────────────────────────────────────────

@app.command(name="factor-ic")
def factor_ic(
    symbol: str = typer.Option("AAPL", help="Symbol for IC analysis"),
    start: str = typer.Option("2021-01-01", help="分析开始日期"),
    end: str = typer.Option(str(date.today()), help="分析结束日期"),
    forward_days: int = typer.Option(5, help="预测未来N日收益"),
):
    """IC因子分析：验证哪些因子对未来收益有真实预测能力。"""
    from factor import FactorEngine, ICAnalyzer

    fetcher = DataFetcher()
    df = fetcher.get_kline(symbol, start, end)

    if len(df) < 150:
        console.print("[red]数据不足，请选择更长的时间段（至少2年）[/red]")
        raise typer.Exit(1)

    engine = FactorEngine()
    analyzer = ICAnalyzer(engine)

    with console.status(f"[bold green]正在分析 {symbol} 各因子IC...[/bold green]"):
        result = analyzer.run(df, symbol, forward_days=forward_days)

    if result.n_periods == 0:
        console.print("[red]数据不足以做IC分析[/red]")
        raise typer.Exit(1)

    df_ic = result.summary_df()

    table = Table(
        title=f"{symbol}  因子IC分析（预测{forward_days}日收益  样本{result.n_periods}期）",
        show_lines=True,
    )
    for col in df_ic.columns:
        table.add_column(col)
    ...
    best = result.best_factors()
    if best:
        console.print(f"\n[bold green]有效因子：[/bold green] {' '.join(best)}")
        console.print("[dim]建议：可在 FactorEngine 中提高这些因子的权重[/dim]")
    else:
        console.print("\n[yellow]当前数据下无显著有效因子，建议换更长时间段测试[/yellow]")
...
def factor_backtest(
    ...
    if spy_filter:
        ...
        console.print("[cyan]已启用 SPY 200日均线过滤器：低于均线时持现金[/cyan]")
    ...
    def _run_segment(seg_dates: list, seg_label: str) -> dict:
        ...
        for di, today in enumerate(seg_dates):
            ...
            if di - last_rb >= rebalance_days:
                # SPY 过滤器：均线以下清仓转现金
                if spy_filter and spy_above_ma and not spy_above_ma.get(today, True):
                    if holdings:
                        console.print(f"  [yellow]{str(today)[:10]} SPY 跌破 MA200，清仓转现金[/yellow]")
                        holdings = []
                    last_rb = di
                else:
